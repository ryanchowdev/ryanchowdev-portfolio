...
const onSubmit = async (event) => {
  ...
  // Always use the form element
  const form = event.currentTarget;
  const formData = new FormData(form);

  // Clear the form after capturing its data
  form.reset();
...
function Contact () {
  return (
    <div className="container py-5" id="contact">
      <div className="aero-glass aero-pad text-center lh-lg">
        <h1>Contact</h1>
        <p>
        ...
        <form className="d-flex flex-column align-items-center" onSubmit={onSubmit}>
          ...
          <button type="submit" className="btn btn-success mt-4 py-2 px-3">
